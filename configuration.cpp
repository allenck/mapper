...
#if 0
void Configuration::saveOldSettings()
{
 QSettings* settings;
 ...
 for(int i=0; i< cityList.count(); i++)
 {
  ...
  settings->setValue("mapId", c->mapId);
...
#endif
void Configuration::saveSettings2()
{
...
//    nc->connectionMap.insert(ncn->uniqueId().toString(), ncn);
  ...
  //currConnection = nc;
   ...
   mapId = settings.value("mapId", "DEMO_MAP_ID").toString();
   // if(mapId.isEmpty())
   //     mapId = "DEMO_MAP_ID";
   mapType = settings.value("mapType", "ROADMAP").toString();
...
void Configuration::getSettings2()
{
    QSettings settings;
    qDebug() << settings.fileName();
