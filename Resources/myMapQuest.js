...
function SegmentInfo(segmentId, routeName, segmentName, oneWay, showArrow, color, tracks, dash, routeType, trackUsage, path )
{
  ...
  this.marker = null;
    ...
    this.createLines = function()
    {
        ...
        if(tracks === 2)
        {
          leftLine = L.polyline(path, {color: color, weight: 2, offset: -5}).addTo(map);
            leftLineColor = color;
            setEvents(leftLine);
          rightLine = L.polyline(path, {color: color, weight: 2, offset: +5}).addTo(map);
...
function createSegment_x(segmentId, routeName, segmentName, oneWay, showArrow, color, tracks, dash, routeType, trackUsage, points )
{
    webViewBridge.setDebug("SegmentId "+ segmentId + "usage: "+ trackUsage);
    var linePath = [];
    var line = null;
    ...
    console.log("line is typeof " + typeof(line) + " " + line)
    if (line instanceof SegmentInfo) {
            console.log("This is a SegmentInfo");
    }
    line.setMap(map);
    ...
    if(arguments.length > 8)
    {
        ...
        for(var i=0; i < points; i +=2)
        {
            path.push(L.latLng(arguments[i+11], arguments[i+12]));
        }
        if(grayLine)
        {
            ...
            for(var i=0; i < points; i +=2)
            {
                grayPath.push(L.latLng(arguments[i+11], arguments[i+12]));
            }
        }
    ...
    if(typeof points == 'array')
    {
        ...
        {
          path.push(L.latLng(points[i], points[i+1]));
        }
        if(grayLine)
        {
...
function SegmentInfo_x(SegmentId, routeName, segmentName, oneWay, showArrow, Color, tracks, dash, routeType, trackUsage )
{
    this.type = "SegmentInfo";
    this.line = null;
    this.path = null;
    ...
    this.placeArrow = function (path)
    {
     ...
     if(path.getLength() > 1)
     {
      ...
      {
      var len = path.getLength();
      var brng =  bearing(path.getAt(len-1).lat(), path.getAt(len-1).lng(),path.getAt(len-2).lat(), path.getAt(len-2).lng() );
      var left = pointRadialDistance( L.latLng(path.getAt(len-1).lat(), path.getAt(len-1).lng()), brng.getBearing()-15, .020);
      var right = pointRadialDistance( l.latLng(path.getAt(len-1).lat(), path.getAt(len-1).lng()), brng.getBearing()+15, .020);
      this.setArrow( myArrow(left.lat(), left.lng(), path.getAt(len-1).lat(), path.getAt(len-1).lng(), right.lat(), right.lng(), currSegment.getColor(), currSegment.segmentId));
      }
     }
     return;
    }  // end placeArrow()
