...
INSERT INTO t1_backup SELECT route, TRIM(routePrefix), routeAlpha, baseRoute, lastUpdate FROM altRoute;
CREATE TEMPORARY TABLE t2_backup(route, `RouteId`,StartDate,endDate,LineKey, OneWay, TrackUsage,
...
INSERT INTO t2_backup SELECT route, `RouteId`,StartDate,endDate,LineKey, OneWay, TrackUsage,
                             CompanyKey, TractionType,Direction,
...
CREATE TABLE `Routes` (  `Route` int(11) NOT NULL,  `RouteId` integer,
...
INSERT INTO Routes (route, `RouteId`,StartDate,endDate,LineKey, OneWay, TrackUsage, CompanyKey, TractionType,Direction,
             next, prev,nextR,prevR,normalEnter,normalleave, reverseEnter, reverseLeave, 
             sequence, reverseSeq,lastUpdate)
             SELECT route, `RouteId`,StartDate,endDate,LineKey, OneWay, TrackUsage, CompanyKey, TractionType,Direction,
             next, prev,nextR,prevR,normalEnter,normalleave, reverseEnter, reverseLeave, 
