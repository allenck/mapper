    ...
    , m_server(server)
{
    ...
    connect(server, SIGNAL(serverError(QWebSocketProtocol::CloseCode)), this,
      SLOT(handleError(QWebSocketProtocol::CloseCode)));
}
...
{
 emit clientConnected(_transport = new WebSocketTransport(m_server->nextPendingConnection()));
 connect(m_server, SIGNAL(closed()), this, SLOT(handleClose()));
 connect(_transport, SIGNAL(readChannelFinished()), SLOT(handleClose()));
}

void WebSocketClientWrapper::handleClose()
{
 emit clientClosed();
}

void WebSocketClientWrapper::handleError(QWebSocketProtocol::CloseCode)
{
 qDebug()<< "WebSocketClientWrapper error";
}

WebSocketTransport* WebSocketClientWrapper::transport() { return _transport;}
