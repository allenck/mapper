...
void SplitSegmentDlg::common(int segmentId)
{
 ...
 connect(ui->cbTrackType1, &QComboBox::currentIndexChanged,[=](int){
  canBeChanged();
 });
 connect(ui->cbTrackType2, &QComboBox::currentIndexChanged,[=](int){
  canBeChanged();
 });
 ...
 connect(ui->cbCompany1, &QComboBox::currentIndexChanged, [=](int ix){
  if(ix < 0)
   return;
 ...
 connect(ui->cbCompany2, &QComboBox::currentIndexChanged, [=](int ix){
  if(ix < 0)
   return;
 ...
 connect(ui->cbTractionType1, &QComboBox::currentIndexChanged,[=](int){
  canBeChanged();
 });
 connect(ui->cbTractionType2, &QComboBox::currentIndexChanged,[=](int){
  canBeChanged();
 });
