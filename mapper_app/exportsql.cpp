   ...
   for(int i=0; i< columns.count(); i++)
   {
    ...
    if(tgtConn->servertype() != "PostgreSQL")
    {
        ...
        if(types.at(i)=="timestamp")
        {
            ...
            qint64 dtEpoch =dt.toMSecsSinceEpoch();
            outStr = QString("to_timestamp(%1)").arg(dtEpoch);
        }
        else
        ...
        if(types.at(i).startsWith("char",Qt::CaseInsensitive)!=0)
        {
            ...
            if(types.at(i).compare("text",Qt::CaseInsensitive)==0 || types.at(i).startsWith("varchar",Qt::CaseInsensitive))
        {
            QString str =query->value(i).toString();
            str = str.replace("'","''");
            outStr = "'" + str +"'";
