...
/*static*/ /*final*/ int SystemConsole::STD_ERR = 1;
/*static*/ /*final*/ int SystemConsole::STD_OUT = 2;
/*private*/ /*static*/ SystemConsole* SystemConsole::_instance = NULL;
 ...
 */
/*public*/  /*static*/ QMainWindow* SystemConsole::getFrame(QWidget* parent) {
    return SystemConsole::getInstance()->openFrame(parent);
}

/*public*/  QMainWindow* SystemConsole::openFrame(QWidget* parent)
{
 ...
 */
/*private*/ void SystemConsole::createFrame(QWidget* parent)
{
    //Q_ASSERT(parent != NULL);
 // Use a JmriJFrame to ensure that we fit on the screen
 frame = new QMainWindow(parent);
 frame->setWindowTitle(tr("System console"));
...
// });
 ...
 //((QMainWindow*)frame)->setAlwaysOnTop(alwaysOnTop->isChecked());
...
{
 //((QMainWindow*)frame)->setAlwaysOnTop(b);
...
void SystemConsole::closeConsole()
{
 frame->close();
 this->deleteLater();
}

void SystemConsole::setAlwaysOnTop(bool b)
{
 Qt::WindowFlags flags = frame->windowFlags();

 if(b)
 {
  frame->setWindowFlags(flags | (Qt::CustomizeWindowHint | Qt::WindowStaysOnTopHint));
  frame->show();
 }
 else
 {
